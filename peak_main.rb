...
if ARGV.size == 0
    ...
    # limit of 50 arbitrarily chosen to keep from slowing down
    runs_db = dbh.execute("SELECT id FROM runs WHERE processed = 0 LIMIT 50")
    # test query ############################################################################################################################
    #runs_db = dbh.execute("select id from runs where date_part('year', sampled_at) = 2012 and date_part('month', sampled_at) = 02 limit 50")
    #########################################################################################################################################
    print "Processing "
